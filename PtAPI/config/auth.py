...
def obter_usuario_atual(
    ...
    """
    Dependency para obter o contratado (usuário) atual autenticado.
    Usar em rotas que requerem autenticação.
    
    Exemplo:
        @router.get("/protegida")
        def rota_protegida(usuario_atual: Contratado = Depends(obter_usuario_atual)):
    ...
    contratado = db.query(Contratado).filter(Contratado.id == usuario_id).first()
